class BootLoader
            ...
            xhr = new XHR @json_url,
              ...
              failure: (status, response)=>
                if URI(@json_url).hostname() == "livepreview.appgyver.com"
                  @fail_with_message "Connection error", "Could not open a connection between AppGyver Cloud and the Preview app. Please ensure that your app ID and PIN are correct. If the problem persists, try restarting the Preview app."
                else
                  @fail_with_message "Connection error", "Could not open a connection between AppGyver Studio and the Preview app. Please check that both are in the same network. If the problem persists, try restarting the Preview app and/or AppGyver Studio."
    else
    ...
    @send "downloadFile",
      ...
      callbacks:
        ...
        failure: ()=>
          ...
          @fail_with_message "Loading the app failed", "Could not download all of the app archives. Please try again. If the problem persists, try restarting the Preview app."
    ...
    @send "unzip",
      ...
      callbacks:
        success: ()=>
          @archives_to_extract--
        failure: ()=>
          @fail_with_message "Loading the app failed", "Could not extract all of the app archives. Please try again. If the problem persists, try restarting the Preview app."
      ...
      @send "downloadFile",
        ...
        callbacks:
          success: ()=>
            @files_to_download--
          failure: ()=>
            @fail_with_message "Loading the app failed", "Could not download all of the app files. Please try again. If the problem persists, try restarting the Preview app."
  ...
  observe_ui: ()=>
    document.addEventListener "DOMContentLoaded", ()=>
      ...
      previous_qr_code.addEventListener "touchstart", ()=>
        if @previousLink
