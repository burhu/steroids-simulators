  ...
  BootLoader = (function() {
    ...
    BootLoader.prototype.get_ip_address = function(callback) {
      ...
      return this.send("getIPAddress", {
        callbacks: {
          ...
          failure: function() {
            if (_this.debug) {
              return console.log("Could not load endpoint URL.");
            }
          }
        }
      });
    ...
    BootLoader.prototype.select_correct_ip = function(ip_addresses, callback) {
      ...
      return this.get_ip_address(function(current_ip) {
        var block_regex, ip, _i, _j, _k, _len, _len1, _len2;
        if (_this.debug) {
          console.log("Comparing to local ip: " + current_ip);
        }
        ...
        block_regex = /^([0-9]+\.[0-9]+)\.[0-9]+\.[0-9]+$/;
        for (_j = 0, _len1 = ip_addresses.length; _j < _len1; _j++) {
          ip = ip_addresses[_j];
          if (_this.debug) {
            console.log("comparing " + (ip.match(block_regex)[1]) + " to " + (current_ip.match(block_regex)[1]));
          }
        ...
        block_regex = /^([0-9]+)\.[0-9]+\.[0-9]+\.[0-9]+$/;
        for (_k = 0, _len2 = ip_addresses.length; _k < _len2; _k++) {
          ip = ip_addresses[_k];
          if (_this.debug) {
            console.log("comparing " + (ip.match(block_regex)[1]) + " to " + (current_ip.match(block_regex)[1]));
          }
        ...
        if (_this.debug) {
          console.log("Could not find matching IP");
        }
    ...
    BootLoader.prototype.start = function(callback) {
      ...
      if (!this.already_started) {
        ...
        if (this.debug) {
          return console.log("User double-tapped, prevented this!");
        }
      }
    };
    ...
    BootLoader.prototype.handle_archives = function() {
      ...
      if (this.debug) {
        console.log("Composing an array of archives to download...");
      }
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        archive = _ref[_i];
        if (archive.url != null) {
          _results.push(this.download_archive(archive));
        }
      }
      return _results;
    };
    ...
    BootLoader.prototype.download_archive = function(archive) {
      var my_uid, name,
        _this = this;
      if (this.debug) {
        console.log("Handling the app archives...");
      }
      my_uid = this.uid++;
    ...
    BootLoader.prototype.disable_view = function(title, message) {
      ...
      if (this.finish_interval != null) {
        return clearInterval(this.finish_interval);
      }
    };
    ...
    BootLoader.prototype.message_handler = function(e) {
      ...
      if ((data != null ? data.callback : void 0) != null) {
        if (this.debug) {
          console.log("Websocket responded with callback");
        }
    ...
    BootLoader.prototype.send = function(method, options) {
      var callback_name, callbacks, request,
        _this = this;
      if (this.debug) {
        console.log("Sending websocket " + method);
      }
      if ((options != null ? options.callbacks : void 0) != null) {
        if (this.debug) {
          console.log("Preparing callbacks");
        }
      ...
      if (this.debug) {
        console.log("Generating request JSON");
      }
